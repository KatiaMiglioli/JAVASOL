...
public class GameState {
    ...
    public boolean jogoGanho() {
        boolean jogoGanho = this.deck.isEmpty() && this.cartaRevelada.isEmpty();
        if( jogoGanho )
            for( int i = 0; i < Solitaire.SOL_STACK_CNT && jogoGanho; i++ )
                jogoGanho = jogoGanho && solStack[ i ].isEmpty();
        return( jogoGanho );
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.deck.getCards().size()!=gs.deck.getCards().size() ||
                this.cartaRevelada.getCards().size()!= gs.cartaRevelada.getCards().size()){
            return false;
        }
    ...
    public ArrayList<GameState> legalMoves (boolean verbose){
        ...
        for (int i = 0; i < this.cartaRevelada.cardCount(); i++) {
            ClassicCard cc=(ClassicCard) (this.cartaRevelada.elementAt(i));
            cc.setLegal(false);
        }
        ...
        for (int i = 0; i < Solitaire.SOL_STACK_CNT; i++) {
            ...
            if (j!=-1){
                ...
                moverEntrePilhas(legalGs, c, i);
                // Can this card be moved to a sequential stack?
                moverParaPilhasSeq(legalGs, c, i);
    ...
    private void moverEntrePilhas(ArrayList<GameState> legalGs, ClassicCard c, int i) {
    ...
    private void moverParaPilhasSeq(ArrayList<GameState> legalGs, ClassicCard c, int i) {
        Stack curr;
        Stack currStack;
        Stack src;
        Stack dst;
    ...
    private void moverReveladasParaPilha(ArrayList<GameState> legalGs, ClassicCard c) {
        Stack curr;
        Stack currStack;
        Stack src;
        Stack dst;
        if (this.cartaRevelada.isEmpty())
            return;
        src = this.cartaRevelada;
        currStack = src.pop( c ); 
        currStack.reverse(); 
        curr = currStack;   
        if( curr != null )
    ...
    private void moverReveladasParaSeq(ArrayList<GameState> legalGs, ClassicCard c) {
        Stack curr;
        Stack currStack;
        Stack src;
        Stack dst;
    ...
    public GameState(   GameInfo gameInfo,
        ...
        this.cartaRevelada=new Stack();
        for (int i = 0; i < cartaRevelada.cardCount(); i++) {
            ClassicCard currentCard = ((ClassicCard)cartaRevelada.elementAt(i));
            ClassicCard c=new ClassicCard(currentCard);
            this.cartaRevelada.push(c);
        }
    ...
    public GameState( GameInfo gameInfo, ClassicDeck deck, Stack cartaRevelada, SolitaireStack[] solStack, SequentialStack[] seqStack) {
        super();
    ...
    public void restaurarEstadoJogo( GameInfo gameInfo, ClassicDeck deck, Stack cartaRevelada, SolitaireStack[] solStack, SequentialStack[] seqStack, Move move) {
        gameInfo.setType( this.gameInfo.getType() );
        gameInfo.setSeed( this.gameInfo.getSeed() );
        for( ; !cartaRevelada.isEmpty(); ){
            cartaRevelada.pop();
        }
        for (int i = 0; i < this.cartaRevelada.cardCount(); i++) {
            ClassicCard currentCard = ((ClassicCard)this.cartaRevelada.elementAt(i));
            ClassicCard c=new ClassicCard(currentCard);
            cartaRevelada.push(c);
