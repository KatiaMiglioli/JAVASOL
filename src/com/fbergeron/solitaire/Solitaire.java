...
public class Solitaire extends Frame
{
    ...
    public void newGame() {
        ...
        cartaRevelada = new Stack();
        cartaRevelada.setLocation( REVEALED_CARDS_POS.x, REVEALED_CARDS_POS.y );
    ...
    public void getNewCards() {
    	...
    	//Contar movimento
    	if(!deck.isEmpty() || !cartaRevelada.isEmpty()) {
    		counter++;
    	}
    	...
    	if( deck.isEmpty() ) {
            for( ; !cartaRevelada.isEmpty(); ) {
                ClassicCard c = ((ClassicCard)cartaRevelada.pop());
                c.turnFaceDown();
                deck.push( c );
            }
        }
        ...
        pushGameState( new GameState( gameInfo, deck, cartaRevelada, solStack, seqStack, null, null, null ) );

        // Flag which cards can be moved legally
        GameState gs = new GameState( gameInfo, deck, cartaRevelada, solStack, seqStack );
    ...
    private boolean jogoGanho() {
        boolean jogoGanho = deck.isEmpty() && cartaRevelada.isEmpty();
        if( jogoGanho )
            for( int i = 0; i < SOL_STACK_CNT && jogoGanho; i++ )
    ...
    class RestartListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            gameStates.get( 0 ).restaurarEstadoJogo( gameInfo, deck, cartaRevelada, solStack, seqStack, null );
    ...
    class UndoListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            if( gameStates.size() - 2 >= 0 ) {
                gameStates.get( gameStates.size() - 2 ).restaurarEstadoJogo( gameInfo, deck, cartaRevelada, solStack, seqStack, null );
                popGameState();
                counter++;
            }
            GameState gs = new GameState( gameInfo, deck, cartaRevelada, solStack, seqStack );
        ...
        public void mousePressed(MouseEvent e) {
            if( !e.isMetaDown() && !e.isControlDown() && !e.isShiftDown() ) {
                ...
                if( deck.contains( p ) )
                    ...
                    if( !cartaRevelada.isEmpty() && cartaRevelada.top().contains( p ) ) {
                        src = cartaRevelada;
    ...
    class Table extends Canvas
    {
        ...
        public void paint( Graphics g ) {
            ...
            //Draw cartaRevelada
            if( cartaRevelada != null && !cartaRevelada.isEmpty() )
                cartaRevelada.top().paint( offscreenGr, menuItemHint.getState() );
